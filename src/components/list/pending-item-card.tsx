...
const priorityPillBg: {
  [key in GroceryItem["priority"]]: string;
} = {
  low: "bg-priority-low",
...
const PendingItemCard = ({ item }: PendingItemCardProps) => {
      ...
      <View className="flex-row items-start gap-3">
        ...
        <View className="flex-1">
          <View className="flex-row items-center justify-between gap-2">
            <Text className="flex-1 text-lg font-semibold font-family-bold text-card-foreground">
              {item.name}
            </Text>
            <View
              className={cn(
                "rounded-full px-3 py-1",
                priorityPillBg[item.priority],
              )}
            >
              <Text
                className={cn(
                  "text-xs font-bold uppercase font-family-bold",
